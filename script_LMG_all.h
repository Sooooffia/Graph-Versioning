...
//    output_file.open(output_name);
    ...
    for (double beta = 1; beta <= 2; beta += 0.1) {
        ...
        output_file << duration_cast<microseconds>(end_LMG-start_LMG).count() << " " << duration_cast<microseconds>(end_LMGA-start_LMGA).count() << endl;
    }
    graph_file.close();
    output_file.close();
}
